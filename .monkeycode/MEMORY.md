...
[智能保活改为加速度模长判定]
- Date: 2026-05-01
- Context: 用户要求不要注册 6 种传感器，改为仅使用加速度传感器，并用加速度向量模长判断静止与运动
- Instructions:
  - 智能保活使用 Android `SensorManager`，但只注册 `TYPE_ACCELEROMETER`。
  - 继续使用 `SensorManager.SENSOR_DELAY_NORMAL` 作为回调频率。
  - 用加速度向量模长 `sqrt(x² + y² + z²)` 判断静止与运动。
  - 模长在 `9.5` 到 `10.5` 之间时判定为静止，不在该区间时判定为运动。
  - 智能保活相关实现和文案应按该加速度模长规则对齐。
